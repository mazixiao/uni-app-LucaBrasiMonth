<template>
	<view>
		...
		<headers title="活动报名" :show_logo="true" :show_bol="false" :show_title="true" titleColor="#fff" :GoBackWhite="true" :backgroundColor="'linear-gradient(135deg,#9c528a, #d7778c 99%)'"></headers>
		...
		<!-- 		<header title="活动报名" show_logo="{{true}}" show_bol="{{false}}" show_title="{{true}}" titleColor="#fff" GoBackWhite="{{true}}"
		 backgroundColor="{{'linear-gradient(135deg,#9c528a, #d7778c 99%)'}}"></header> -->
					...
					<view class="Venue-wrap">
						<view class="Venue">
							...
							<view class="list">
								...
								<!-- <view class="text">参会日期：{{dateTransform.sub(item.participantstime)}}</view> -->
							</view>
						</view>
						<view class="check" @click="checkFun(item.id)">查看</view>
					</view>
					<!-- 是否过期 0是 1否 -->
